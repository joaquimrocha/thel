...
plugins, and stubbed app commands (PTY, git, backend availability, …).
...
integration tests that drive real `git`
and `the backend`). CI runs both jobs.
...
- `gotoApp(page, config?)` installs the mock, opens the app, and waits for
  hydration. `config` can set `{ backendAvailable, liveTerminals, label }`.
